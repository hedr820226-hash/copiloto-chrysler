...
def transcribe():
    """Recibe audio del navegador y lo convierte a texto con Groq Whisper."""
    if not client:
        return jsonify({
            "error": "Sistema de voz no disponible: falta GROQ_API_KEY"
        }), 503
    ...
    try:
        # Evita enviar archivos absurdamente grandes al servicio de voz.
        ...
        texto = getattr(transcription, "text", "") or ""
...
# =====================================
# 📱 API CHAT CON ARCHIVOS
# =====================================

@app.route('/chat/file', methods=['POST'])
def chat_file():
    """
    Mantiene compatible el endpoint que usa index.html.
    La conversación de texto se procesa igual que /chat.
    """
    try:
        data = request.get_json(force=True) or {}
        texto = data.get("mensaje") or data.get("message") or ""
