...
def generar_respuesta(
    ...
    # -----------------------------
    if archivo_adjunto:
        nombre = archivo_adjunto.get("name", "archivo")
        tipo = archivo_adjunto.get("type", "")
        datos = archivo_adjunto.get("base64", "")
        ...
        if tipo.startswith("image/"):
            ...
            contenido_usuario = [
                {
                    "type": "text",
                    "text": f"Analiza esta imagen.\n\nArchivo:\n{nombre}\n\nPregunta:\n{texto}"
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:{tipo};base64,{datos}"
                    }
                }
            ]
        else:
            try:
                ...
                contenido_usuario = f"El usuario envió este archivo:\n\n{nombre}\n\nContenido:\n\n```text\n{codigo}\n```\n\nPetición: {texto}"
            except Exception as e:
                logging.error("Error al procesar archivo de texto: %s", e)
                contenido_usuario = f"[Error al leer el archivo {nombre}]. {texto}"
    ...
    palabras_codigo = [
        "codigo", "código", "programar", "python", "java", "android",
        "html", "css", "javascript", "error", "compile", "flask", "api"
    ]
    ...
    try:
        ...
        if not texto_respuesta:
            texto_respuesta = "No pude generar una respuesta."

        # =====================================
        # 💾 GUARDAR MEMORIA
        # =====================================
        if archivo_adjunto:
            memoria_usuario = f"[Archivo enviado: {archivo_adjunto.get('name', 'archivo')}] {texto}"
        else:
            if len(texto) > 500:
                memoria_usuario = "[Mensaje largo] " + texto[:200]
            else:
        ...
        logging.error("ERROR GROQ: %s", e)
        return "Dash tuvo un problema procesando la solicitud."
...
def limpiar():
    datos = request.get_json() or {}
    session_id = datos.get("session_id") or ""

    if not session_id:
        return jsonify(
            {
                "response": "No existe sesión."
            }
        )
    ...
    return jsonify(
        {
            "response": "Memoria de Dash limpiada."
        }
    )
